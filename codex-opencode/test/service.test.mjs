...
import { fixture, task, cli, fake, root } from './helpers.mjs';
...
test('authenticated daemon survives MCP disconnect and restart without duplicate prompts',async t=>{
  ...
  const result=await client.callTool({name:'submit_tasks',arguments:{request_id:'single',repo:f.repo,tasks:[task('a',{files:{'a.txt':'a'},delay:3000,drop_response:true})]}});
  const submitted=JSON.parse(result.content[0].text); assert.ok(submitted.run_id); await client.close();
  let state;
  for(let i=0;i<100;i++){state=new Store(f.data).state;if(state.runs[0].tasks[0].attempts[0]?.worker?.prompt_intent)break;await sleep(100);}
  ...
  for(let i=0;i<150;i++){state=new Store(f.data).state;if(state.runs[0].tasks[0].state==='needs_review')break;await sleep(100);}
  ...
  const repeat=await remoteCall(f.data,cli,'submit_tasks',{request_id:'single',repo:f.repo,tasks:[task('a',{files:{'a.txt':'a'},delay:3000,drop_response:true})]}); assert.equal(repeat.run_id,submitted.run_id);
});

test('installed bundle works after its source is moved, without source node_modules',async t=>{
  const dir=fs.mkdtempSync(path.join(os.tmpdir(),'설치 경로 space ')); const source=path.join(dir,'개발 source'),install=path.join(dir,'설치본'),data=path.join(dir,'state');fs.mkdirSync(source);
