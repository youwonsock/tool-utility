...
- Windows cancellation/restart reached confirmed cancellation after the test's former 15-second wait. Its wait now allows 60 seconds for process-tree/CIM checks and also verifies that the actual OpenCode server is no longer alive. The complete rerun is pending; the Unix process-group-specific test is skipped on Windows, whose worker cancellation exercises `taskkill /T`.
